...
function DoctorUpdateForm({ setState,doctor}:{setState:(status:boolean)=>void,doctor:Doctor}):JSX.Element {
    ...
    e.preventDefault()
    dispatch(updateDoctors({id:doctor.id,firstName,secondName, patronymic,description, img, slot}))
    setState(false)
    }
  return (
    ...
    <input name='patronymic' type='text' value={patronymic} onChange={(e)=>setPatronymic(e.target.value)}/>
