import {
   ...
   export const initDoctors = createAsyncThunk(
    'doctors/init',
    () =>  api.initDoctorFetch()
   );
   export const addDoctors = createAsyncThunk(
    'doctors/add',
    (obj:FormData) =>  api.addFetchDoctor(obj)
   );
   export const deleteDoctors = createAsyncThunk(
    'doctors/delete',
    (id:IdDoctor) =>  api.delFetchDoctor(id)
   );
   export const updateDoctors = createAsyncThunk(
    'doctors/update',
    (obj:Doctor) =>  api.updateFetchDoctor(obj)
   );
    ...
    extraReducers: (builder) => {
      builder
        ...
        .addCase(updateDoctors.fulfilled, (state, action) => {
          state.doctors = state.doctors.map(doctor=>doctor.id !== action.payload.id? doctor: action.payload);
      })
